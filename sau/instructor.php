...
<?php
	...
	if(isset($_GET['instructorId']) && isset($_GET['imageId'])){
	...
	<div class="container" style="border-radius:5px; border:1px solid; background-color:#FFFFFF; padding:2%; padding-top:1%">
		...
		<div class="col-md-12">
			...
			<div class="col-md-9" style="padding:1%">
				<label>Öğretim Elemanı Bilgileri</label>
				<div class="col-md-12" style="border:0.2px solid; background-color:#FFFFFF"></div>
			...
			<table class="table table-striped" style="margin-top:1%">
    			...
    			<tbody>
    				<?php
    					...
    					ON lesson.scheduleId=schedule.id INNER JOIN class ON lesson.classId=class.id WHERE lesson.instructorId='$id'";
    					$result = $con->query($sql);
    					while($row = $result->fetch_assoc()){
    						echo "<tr>";
    						echo "<td><a href=\"lesson.php?lessonId=".$row['id']."\">".$row['lessonName']."</a></td>";
    						echo "<td>".$row['term']."</td>";
    						echo "<td>".$row['day']."</td>";
