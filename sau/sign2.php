...
<?php
	...
	if( isset($_POST["studentId"]) & isset($_POST["major"]) & isset($_POST["minor"]) ){
		...
		if($result1->num_rows > 0){//Bu major ve minor değerlerine ait sınıf var ise
			...
			$sql2 = "SELECT lesson.id FROM lesson INNER JOIN schedule ON lesson.scheduleId=schedule.id WHERE CURRENT_TIME() BETWEEN schedule.startTime AND schedule.endTime AND schedule.day = DAYNAME(CURRENT_DATE()) AND lesson.classId = '$classId'";
			...
			if($result2->num_rows > 0){//O anda o sınıfta bir ders var ise
				...
				$sql = "SELECT lesson.lessonName,lesson.term,instructor.firstName,instructor.lastName,instructor.rank FROM lesson INNER JOIN instructor ON lesson.instructorId=instructor.id WHERE lesson.id = '$lessonId'";
				$result = $con->query($sql);
				$row = $result->fetch_assoc();

				$lessonName = utf8_encode($row['lessonName']);
